...
module modules
{
    export class DragonBonesFastArmature
    {
        ...
        {
            if(!DragonBonesFastArmature.dragonbonesFactory)//相同数据内容的龙骨工厂不需要创建2次或2次以上，创建1次后可以持有其引用，用于继续创建副本
            {
            ...
            var dragonbonesFactory: dragonBones.EgretFactory = DragonBonesFastArmature.dragonbonesFactory;
            ...
            var anmfs:dragonBones.FastAnimationState = armature.animation.gotoAndPlay("射击",-1);//⑤参数1是动作关键帧标签名称，参数2是切换动作淡入（自动完成过渡动画补间）时间：默认值-1表示使用默认的数据时间 。淡入可以使动作细腻，但是容易造成骨骼“走位”。
