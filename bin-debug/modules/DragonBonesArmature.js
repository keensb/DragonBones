...
(function (modules) {
    var DragonBonesArmature = (function () {
        function DragonBonesArmature(displayContainer) {
            ...
            if (!DragonBonesArmature.dragonbonesFactory) {
                //注意有5个名称不能写错，还要注意大小写
            ...
            var dragonbonesFactory = DragonBonesArmature.dragonbonesFactory;
            ...
            var anms = armature.animation.gotoAndPlay("win", -1); //⑤参数1是动作关键帧标签名称，参数2是切换动作淡入（自动完成过渡动画补间）时间：默认值-1表示使用默认的数据时间 。淡入可以使动作细腻，但是容易造成骨骼“走位”。
    ...
    modules.DragonBonesArmature = DragonBonesArmature;
    egret.registerClass(DragonBonesArmature,'modules.DragonBonesArmature');
})(modules || (modules = {}));
