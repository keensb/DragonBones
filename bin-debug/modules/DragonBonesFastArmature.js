...
var modules;
(function (modules) {
    var DragonBonesFastArmature = (function () {
        function DragonBonesFastArmature(displayContainer) {
            if (!DragonBonesFastArmature.dragonbonesFactory) {
                ...
                //创建龙骨动画工厂对象
                DragonBonesFastArmature.dragonbonesFactory = new dragonBones.EgretFactory();
                //解析骨骼动画信息, 并添加到龙骨动画工厂的数据列表
                DragonBonesFastArmature.dragonbonesFactory.addDragonBonesData(dragonBones.DataParser.parseDragonBonesData(dragonbonesData));
                //把骨骼动画所需要的纹理集资源添加到工厂纹理集列表中
                DragonBonesFastArmature.dragonbonesFactory.addTextureAtlas(new dragonBones.EgretTextureAtlas(texture, textureData));
            }
            var dragonbonesFactory = DragonBonesFastArmature.dragonbonesFactory;
            //创建一个可视的骨架FastArmature对象(注意：是骨“架”，不是骨“骼”，骨骼是构成骨架的基本元素)
            //一般情况下出现报错 'display' of null 是因为骨架名称有误
            var armature = dragonbonesFactory.buildFastArmature("armatureName"); //④骨架名称(这个最容易被忽略)
            ...
            var anmfs = armature.animation.gotoAndPlay("射击", -1); //⑤参数1是动作关键帧标签名称，参数2是切换动作淡入（自动完成过渡动画补间）时间：默认值-1表示使用默认的数据时间 。淡入可以使动作细腻，但是容易造成骨骼“走位”。
        ...
        var d = __define,c=DragonBonesFastArmature,p=c.prototype;
        return DragonBonesFastArmature;
    }());
    modules.DragonBonesFastArmature = DragonBonesFastArmature;
    egret.registerClass(DragonBonesFastArmature,'modules.DragonBonesFastArmature');
})(modules || (modules = {}));
