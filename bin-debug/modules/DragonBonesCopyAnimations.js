...
(function (modules) {
    var DragonBonesCopyAnimations = (function () {
        function DragonBonesCopyAnimations(displayContainer) {
            ...
            //注意有5个名称不能写错，还要注意大小写
            var dragonbonesData = RES.getRes("Robot"); //①骨骼动作文件名称
            var textureData = RES.getRes("Robot_json"); //②纹理集信息文件名称
            var texture = RES.getRes("Robot_png"); //③纹理集图片文件名称
            //创建龙骨动画工厂对象
            ...
            var armature = dragonbonesFactory.buildArmature("robot"); //④骨架名称(这个最容易被忽略)
            //把骨架对象的可视对象添加到舞台
