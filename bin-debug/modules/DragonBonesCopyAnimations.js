...
var modules;
(function (modules) {
    var DragonBonesCopyAnimations = (function () {
        function DragonBonesCopyAnimations(displayContainer) {
            if (!DragonBonesCopyAnimations.dragonbonesFactory) {
            ...
            var dragonbonesFactory = DragonBonesCopyAnimations.dragonbonesFactory;
            //创建一个可视的骨架对象(注意：是骨“架”，不是骨“骼”，骨骼是构成骨架的基本元素)
            //一般情况下出现报错 'display' of null 是因为骨架名称有误
            var armatureRobot = dragonbonesFactory.buildArmature("robot"); //④骨架名称(这个最容易被忽略)
            //把骨架对象的可视对象添加到舞台
            ...
            var anmsRobot = armatureRobot.animation.gotoAndPlay("oneLegStand2", -1); //⑤参数1是动作关键帧标签名称，参数2是切换动作淡入（自动完成过渡动画补间）时间：默认值-1表示使用默认的数据时间 。淡入可以使动作细腻，但是容易造成骨骼“走位”。
            ...
            var armatureWarrior = dragonbonesFactory.buildArmature("warrior"); //④骨架名称(这个最容易被忽略)
            ...
            var slot = armatureWarrior.getSlot("innerweapon");
            ...
            var anmsWarrior = armatureWarrior.animation.gotoAndPlay("oneLegStand2"); //⑤动画关键帧标签名称
        ...
        var d = __define,c=DragonBonesCopyAnimations,p=c.prototype;
        return DragonBonesCopyAnimations;
    }());
    modules.DragonBonesCopyAnimations = DragonBonesCopyAnimations;
    egret.registerClass(DragonBonesCopyAnimations,'modules.DragonBonesCopyAnimations');
})(modules || (modules = {}));
